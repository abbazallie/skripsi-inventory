        ...
        <div class="card-body">
            ...
            <div class="form-row">
                ...
                <label for="nb">Nama Barang</label>
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Nama Barang" id="nb" value="{{ old('name', $item->name) }}">
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group col-md-3">
                    ...
                    @foreach ($categories as $ktg)
                        <option value="{{ $ktg->id }}" {{ old('category_id',$item->category_id) == $ktg->id ? 'selected' : null}}>{{ $ktg->name }}</option>
                    @endforeach
                    </select>
                    @error('kategori_id')
                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="form-row">
                ...
                <label for="jm">Jumlah</label>
                    <input type="number" name="amount" class="form-control @error('amount') is-invalid @enderror"  placeholder="Jumlah Barang" id="jm" value="{{ old('amount',$item->getStock()) }}">
                    @error('amount')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                </div>
                ...
                <label for="ru">Ruang/Tempat</label>
                    ...
                    <select name="place_id" id="selectPlace" class="form-control form-control-sm"  @error('place_id') is-invalid @enderror" name="place_id">
                        @foreach (App\Models\Place::all() as $place)
                            <option value="{{ $place->id }}" {{ old('place_id',$item->place_id) == $place->id ? 'selected' : null}}>{{ $place->name }}</option>
                        @endforeach
                    </select>
                    @error('place_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input name="date_in" type="date" class="form-control @error('date_in') is-invalid @enderror"  value="{{ old('date_in',$item->date_in) }}">
                    @error('date_in')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
